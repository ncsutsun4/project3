...
velData <- InPlay %>%  select(pitchType, releaseVelocity) %>%
     filter(pitchType %in% fastballs)


unique(velData$pitchType)

ggplot(velData, aes(x = releaseVelocity, fill = pitchType)) +
...
logi <- glm(FF_Type ~ releaseVelocity + spinRate + spinDir, family="binomial", data=modelData)
